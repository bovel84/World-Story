...
- G17 è stato avviato: il protocollo NDJSON distingue ora auto-jump e salto fisso. In auto `targetDate` deve coincidere con l'evento che contiene la decisione NPC che arresta il salto (controparte diretta o misura autonoma `counterAction`); se nessun evento è giustificato, richiede `targetDate:null` senza inventare svolte. L'istruzione auto dichiara esplicitamente di prevalere su regole preset contraddittorie (distribuire eventi/non interrompere). Il contratto è coperto da test prompt.
- Auto-jump «decisione NPC» (opzione B): il salto non si ferma più al primo fatto di cronaca. Prosegue in ordine cronologico attraverso i fatti di contorno e si arresta sulla prima reazione che contiene una decisione concreta verso il giocatore — una controparte diretta (`role: "counterparty"`) oppure una misura autonoma (`counterAction` non vuoto). Gli eventi successivi restano fuori dal checkpoint e le loro chat non vengono aperte, così le conversazioni non compaiono a date diverse da quella di arresto. Il budget eventi è condiviso tra motore e prompt (`core/simulation/EventBudget.ts`: minimo 2, tetto 6, lookahead +2 sugli ordini) e coperto da test puri e di sessione.
- Diplomazia contestualizzata e naturale: le chat ammettono solo nazioni pertinenti alla situazione. Le reazioni automatiche di fallback nascono ora soltanto per le controparti che il giocatore ha davvero interpellato nei suoi ordini (un nome citato di sfondo in un dispaccio non apre un canale) e il prompt elenca esplicitamente i soli presenti, vietando di far parlare o trattare come presenti altri paesi. Il messaggio diplomatico è inoltre vincolato a uno stile naturale: vietate cifre, percentuali, punteggi, livelli, indici, PIL, popolazione, numero di regioni o divisioni e statistiche, che vanno tradotte in giudizi politici («una forza militare superiore», «un'economia fragile»); i dati quantitativi restano nel dossier interno dell'interlocutore, marcati come riservati e mai citabili. Coperto dai test chat e di pertinenza NPC.
- Una chat è una discussione, non un canale permanente: ogni nuovo scambio con una nazione apre una **nuova** chat e le precedenti con gli stessi interlocutori finiscono in **archivio** (consultabili ma fuori dall'elenco attivo, senza pesare sui non letti). Il backend conserva più discussioni per lo stesso insieme di partecipanti: è stato rimosso il vincolo `UNIQUE(game_id, participant_key)`, sostituito da un indice di ricerca e da una deduplica per evento/turno (`dedupe_key`), così i ritentativi dello stesso fatto non duplicano il canale. Le chat archiviate restano nella memoria strategica e nei trascritti del motore, non vengono cancellate. Coperto da `chat-threads.test.ts`, `chats.test.ts` e `chatTimeline.test.ts`.
- Il canale diplomtico parla con la voce della nazione: le reazioni strutturate del simulatore portano ora un campo `note` — il messaggio diretto al giocatore, in prima persona — che apre la chat, mentre `response` e `counterAction` restano il testo di cronaca del dispaccio. Spariscono le etichette da bollettino («Misura annunciata») e il registro giornalistico nei messaggi; se il modello omette `note`, si ripiega sulla decisione senza etichette. Coperto da `chats.test.ts` e `prompts.test.ts`.
- Progetti nazionali leggibili e onesti: il Dossier li raggruppa per ambito (Difesa, Infrastrutture, Economia e industria, Ricerca e tecnologia, Società, Diplomazia, Altro) con classificazione deterministica lato read model (`projectCategory.ts`), e distingue i «Completati» dai progetti in corso. L'avanzamento non resta più bloccato al 99%: alla scadenza dichiarata il progetto tocca il 100% e il motore lo chiude (idempotente rispetto alla chiusura esplicita via `completesProjectId`), passando tra i «Completati» con la data di chiusura. Coperto da `military-production.test.ts`, `order-cost-integration.test.ts` e `projectCategory.test.ts`.
...
- Copertura §15 estesa: aggiunti i test T08 (coda con soli ordini già completati → comportamento identico a zero nuovi ordini, un solo salto) e T24 (due eventi nella stessa data: il salto fisso li applica entrambi con ordinamento; l'auto-jump li applica entrambi sulla medesima data, senza aggiungere un giorno, finché nessuno porta una decisione NPC che arresta il salto).
...
**Stato attuale (a questa data):** i nodi centrali di G02/G03/G04/G05/G06/G11/G12/G17/G19/G22 (lotto unico per salto, coda e processi persistiti, ordini zero senz'ordine fittizio, modifica/rimozione della coda, esiti individuali persisti, endpoint di run/checkpoint, timeline paginata dal registro persistente, Enhance in anteprima e lettore di sessione separato) sono implementati e coperti da test. L'auto-jump attraversa i fatti di contorno e termina sulla prima decisione NPC che risponde agli ordini del giocatore (controparte diretta o misura autonoma); il salto a data esplicita funziona anche senza nuovi ordini; Intervene, Save/Load, Rewind e il ripristino/continuazione da checkpoint sono affidabili.
...
- [ ] L'auto-jump termina sulla prima decisione NPC che risponde agli ordini (controparte diretta o `counterAction`), senza applicare eventi o chat successivi.
