...
- **Output dal motore, non riscritti**: al seed la ricetta di ogni impianto prende gli output da `marginalProduction({ factories | ports | universities: 1 }, endowment, technologies)` — cioè dal profilo che il motore calcola con `advanceStock`. La **somma degli impianti è la produzione nazionale** (a fattore pieno), non una seconda formula.
